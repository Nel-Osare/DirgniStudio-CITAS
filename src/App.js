...
const ADMIN_PIN = "2024"; // PIN para entrar como Admin
      ...
      <header className="bg-white/90 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex bg-gray-100 p-1.5 rounded-2xl border border-gray-200 shadow-inner scale-90">
            <button onClick={() => setView('client')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all duration-300 ${view === 'client' ? 'bg-white text-black shadow-md' : 'text-gray-400'}`}>CITA</button>
            <button onClick={handleAdminAccess} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all duration-300 ${view === 'admin' ? 'bg-black text-amber-500 shadow-lg' : 'text-gray-400'}`}>ADMIN</button>
          </div>
        </div>
      </header>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-xs rounded-[3rem] p-8 shadow-2xl border border-gray-100">
            <div className="flex justify-between items-start mb-6">
               ...
               <button onClick={() => setShowPinModal(false)} className="text-gray-300"><X size={20}/></button>
            </div>
            <h2 className="text-xl font-black uppercase tracking-tighter mb-2">Acceso Privado</h2>
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-6">Introduce tu PIN</p>
            <form onSubmit={verifyPin} className="space-y-4">
              <input autoFocus type="password" maxLength={4} value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="****" className={`w-full text-center text-3xl tracking-[0.5em] py-4 rounded-2xl bg-gray-50 border-2 outline-none transition-all ${pinError ? 'border-red-500' : 'border-transparent focus:border-amber-500'}`} />
              <button type="submit" className="w-full bg-black text-amber-500 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">VERIFICAR</button>
            </form>
          </div>
        </div>
      )}
...
function ClientView({ authError }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Abrir WhatsApp de inmediato para evitar que el cliente espere
    ...
    } catch (err) { console.warn("Error en BD silenciado."); }
  };

  if (isSent) return (
    <div className="text-center py-20 px-6 space-y-6 animate-in fade-in zoom-in duration-500">
      ...
      <p className="text-gray-400 text-sm">Ya enviamos tu mensaje por WhatsApp. Estaremos en contacto pronto.</p>
      <button onClick={() => setIsSent(false)} className="mt-8 text-[10px] font-black uppercase underline text-gray-400">Hacer otra solicitud</button>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-50 space-y-4">
        ...
        <input required type="tel" placeholder="WhatsApp (Celular)" className="w-full p-5 rounded-2xl bg-gray-50 border-0 outline-none focus:ring-2 focus:ring-amber-500/20 transition-all font-medium" onChange={e => setFormData({...formData, phone: e.target.value})} />
      </div>

      {/* Servicios Premium */}
      <div className="grid grid-cols-3 gap-2">
        {[
          {id:'cabello', label:'Cabello', icon:<Scissors size={18}/>}, 
          {id:'maquillaje', label:'Makeup', icon:<Sparkles size={18}/>}, 
          {id:'fotografia', label:'Foto', icon:<Camera size={18}/>}
        ].map(s => (
      ...
      <div className="bg-gray-900 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group">
        ...
        {[1, 2, 3].map(n => <input key={n} required type="datetime-local" className="w-full p-4 mb-3 rounded-2xl border-0 bg-white/10 text-white font-bold outline-none focus:bg-white/20 transition-all relative z-10" onChange={e => setFormData({...formData, [`slot${n}`]: e.target.value})} />)}
      </div>

      <button type="submit" className="w-full bg-black text-amber-500 py-6 rounded-[2.5rem] font-black text-xl shadow-2xl hover:bg-gray-900 active:scale-95 transition-all flex items-center justify-center gap-4 group">
            ...
            <div className="space-y-3">
              {['slot1', 'slot2', 'slot3'].map((k, idx) => (
                <button key={k} onClick={() => confirmDate(app, k)} className="w-full flex justify-between items-center p-5 bg-gray-50 rounded-2xl text-[11px] font-black hover:bg-black hover:text-amber-500 transition-all border border-transparent hover:border-amber-500 group">
                  <span className="flex items-center gap-4"><span className="w-6 h-6 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center text-[9px] font-black group-hover:bg-amber-500 transition-colors">{idx + 1}</span>{app[k] ? new Date(app[k]).toLocaleString('es-CR', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '---'}</span>
