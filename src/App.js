    ...
    const initAuth = async () => {
      try {
        ...
        if (error.code === 'auth/configuration-not-found') {
          setAuthError("Configuración pendiente: Activa acceso anónimo en Firebase.");
        }
      }
    };
    initAuth();
      ...
      <header className="bg-white/90 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-black text-2xl tracking-tighter uppercase italic leading-none">Dirgni Studio</h1>
              <span className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.3em]">Boutique</span>
            </div>
          </div>
          <div className="flex bg-gray-100 p-1 rounded-2xl scale-90">
            <button onClick={() => setView('client')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all ${view === 'client' ? 'bg-black text-amber-500 shadow-md' : 'text-gray-400'}`}>CITA</button>
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-xs rounded-[3rem] p-8 shadow-2xl animate-in zoom-in duration-300">
            ...
            <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-6">Introduce el PIN de acceso</p>
            <form onSubmit={verifyPin} className="space-y-4">
              <input autoFocus type="password" maxLength={4} value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="****" className={`w-full text-center text-3xl py-4 rounded-2xl bg-gray-50 border-2 outline-none transition-all ${pinError ? 'border-red-500 animate-shake' : 'border-transparent focus:border-amber-500'}`} />
...
function ClientView() {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { console.warn("BD ocupada, WhatsApp enviado."); }
  };

  if (isSent) return (
    <div className="text-center py-20 px-6 space-y-6 animate-in fade-in zoom-in">
      ...
      <p className="text-gray-400 text-sm">Se ha abierto WhatsApp para confirmar. ¡Te esperamos!</p>
      <button onClick={() => setIsSent(false)} className="text-[10px] font-black uppercase underline text-gray-400 tracking-widest">Nueva Cita</button>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10 animate-in fade-in duration-700">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-50 space-y-4">
        ...
        <input required placeholder="Nombre" className="w-full p-5 rounded-2xl bg-gray-50 outline-none focus:ring-2 focus:ring-amber-500/20 transition-all font-medium" onChange={e => setFormData({...formData, name: e.target.value})} />
        <input required placeholder="Apellidos" className="w-full p-5 rounded-2xl bg-gray-50 outline-none focus:ring-2 focus:ring-amber-500/20 transition-all font-medium" onChange={e => setFormData({...formData, lastName: e.target.value})} />
        <input required type="tel" placeholder="WhatsApp" className="w-full p-5 rounded-2xl bg-gray-50 outline-none focus:ring-2 focus:ring-amber-500/20 transition-all font-medium" onChange={e => setFormData({...formData, phone: e.target.value})} />
      </div>
      ...
      <div className="bg-gray-900 p-8 rounded-[3rem] shadow-2xl space-y-4">
...
function AdminView({ appointments, loading, onLogout }) {
  ...
  if (loading) return <div className="py-20 flex flex-col items-center justify-center"><div className="w-10 h-10 border-4 border-amber-500 border-t-transparent rounded-full animate-spin"></div></div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div className="flex items-center justify-between px-2">
        <div>
          <h2 className="font-black text-xl tracking-tighter uppercase italic leading-none">Agenda</h2>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Nuevas Solicitudes</p>
        </div>
        <button onClick={onLogout} className="bg-gray-100 p-2.5 rounded-xl text-gray-400 hover:text-red-500 transition-all"><Unlock size={18}/></button>
      </div>

      {pending.length === 0 ? <div className="py-24 text-center text-gray-300 font-black uppercase text-[10px] tracking-widest italic border-2 border-dashed border-gray-100 rounded-[3rem]">Sin pendientes</div> : 
        pending.map(app => (
          <div key={app.id} className="bg-white border border-gray-50 rounded-[2.5rem] p-8 shadow-xl shadow-gray-200/20 space-y-6">
            <div className="flex justify-between items-start">
              <div><h3 className="font-black text-2xl tracking-tighter uppercase leading-none">{app.name} <span className="text-amber-600 font-light">{app.lastName}</span></h3><p className="text-[10px] font-bold text-gray-400 mt-2 tracking-widest">{app.phone}</p></div>
            ...
            <div className="space-y-3 pt-2">
              <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest block ml-2">Confirmar Opción:</span>
              {['slot1', 'slot2', 'slot3'].map((k, idx) => (
                <button key={k} onClick={() => confirmDate(app, k)} className="w-full flex justify-between items-center p-5 bg-gray-50 rounded-2xl text-[11px] font-black hover:bg-black hover:text-amber-500 transition-all border border-transparent hover:border-amber-500 group">
                  <span className="flex items-center gap-4"><span className="w-7 h-7 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center text-[9px] font-black group-hover:bg-amber-500 group-hover:text-black">{idx + 1}</span>{app[k] ? new Date(app[k]).toLocaleString('es-CR', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '---'}</span>
