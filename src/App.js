...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-black font-sans selection:bg-amber-100">
      {authError && (
        <div className="bg-red-600 text-white text-[10px] py-2 text-center font-bold uppercase tracking-widest sticky top-0 z-[100] flex items-center justify-center gap-2">
      ...
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-50 px-6 h-20 flex items-center justify-between max-w-xl mx-auto shadow-sm">
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 scale-90">
          <button onClick={() => setView('client')} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all ${view === 'client' ? 'bg-white text-black shadow-md' : 'text-slate-400'}`}>CITA</button>
          <button onClick={handleAdminAccess} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all ${view === 'admin' ? 'bg-black text-amber-500 shadow-md' : 'text-slate-400'}`}>ADMIN</button>
        </div>
      </nav>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-xs rounded-[3rem] p-8 shadow-2xl animate-in zoom-in duration-300">
            ...
            <form onSubmit={verifyPin} className="space-y-4">
              <input autoFocus type="password" maxLength={4} value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="PIN" className={`w-full text-center text-3xl py-4 rounded-2xl bg-gray-50 border-2 outline-none transition-all ${pinError ? 'border-red-500 animate-shake' : 'border-transparent focus:border-amber-500'}`} />
              <button type="submit" className="w-full bg-black text-amber-500 py-4 rounded-2xl font-black text-xs uppercase shadow-xl">Verificar</button>
            </form>
          </div>
        </div>
      )}
...
function ClientView() {
  ...
  const handleSubmit = (e) => {
    ...
    // 2. ABRIR WHATSAPP DE INMEDIATO (No bloqueante)
    window.open(`https://wa.me/${SALON_PHONE}?text=${encodeURIComponent(msg)}`, '_blank');
    setIsSent(true);

    // 3. Intentar guardar en base de datos en segundo plano
    addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'appointments'), { 
      ...formData, 
      status: 'pendiente', 
      createdAt: serverTimestamp() 
    }).catch(err => console.warn("BD ocupada o sin permisos. WhatsApp enviado correctamente."));
  };

  if (isSent) return (
    <div className="text-center py-20 px-6 space-y-6">
      <div className="w-20 h-20 bg-amber-50 rounded-full flex items-center justify-center mx-auto border border-amber-100 shadow-inner"><CheckCircle className="w-10 h-10 text-amber-600" /></div>
      <h2 className="text-2xl font-black uppercase tracking-tighter italic">¡Solicitud Enviada!</h2>
      <p className="text-slate-400 text-sm">Se ha abierto WhatsApp. Te confirmaremos pronto.</p>
      <button onClick={() => setIsSent(false)} className="text-[10px] font-black uppercase underline text-slate-400">Agendar otra</button>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10 animate-in fade-in duration-700">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/40 border border-slate-50 space-y-4">
        <div className="flex items-center gap-2 mb-2"><User size={14} className="text-amber-600"/><span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Tus Datos</span></div>
        ...
        <input required type="tel" placeholder="WhatsApp" className="w-full p-5 rounded-2xl bg-slate-50 outline-none border-2 border-transparent focus:border-amber-500/20 focus:bg-white transition-all font-medium" onChange={e => setFormData({...formData, phone: e.target.value})} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <button key={s.id} type="button" onClick={() => setFormData({...formData, service: s.id})} className={`p-6 rounded-[2.2rem] border-2 flex flex-col items-center gap-3 transition-all ${formData.service === s.id ? 'border-amber-500 bg-white shadow-xl text-black scale-105' : 'bg-slate-50 border-transparent text-slate-300 grayscale opacity-60'}`}>
            {s.icon} <span className="text-[9px] font-black uppercase tracking-tighter text-center leading-none">{s.label}</span>
          </button>
        ))}
      </div>

      <div className="bg-slate-900 p-8 rounded-[3rem] shadow-2xl space-y-4 relative">
        <div className="absolute top-0 right-0 p-8 opacity-5"><Calendar size={120} className="text-white" /></div>
        <div className="flex items-center gap-2 mb-2 text-amber-500 font-black text-[11px] uppercase tracking-widest relative z-10"><Clock size={16}/> Elige 3 opciones</div>
        {[1, 2, 3].map(n => <input key={n} required type="datetime-local" className="w-full p-4 rounded-xl border-0 bg-white/10 text-white font-bold outline-none focus:bg-white/20 transition-all relative z-10" onChange={e => setFormData({...formData, [`slot${n}`]: e.target.value})} />)}
      </div>

      <button type="submit" className="w-full bg-black text-amber-500 py-6 rounded-[2.5rem] font-black text-xl shadow-2xl flex items-center justify-center gap-4 group">
...
function AdminView({ appointments, loading, onLogout }) {
  ...
  if (loading) return <div className="py-32 flex flex-col items-center justify-center gap-6 animate-pulse"><div className="w-10 h-10 border-4 border-amber-500 border-t-transparent rounded-full animate-spin"></div><span className="font-black text-[10px] text-slate-400 tracking-[0.4em] uppercase">Abriendo Agenda...</span></div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div className="flex items-center justify-between px-2">
        <div><h2 className="font-black text-xl tracking-tighter uppercase italic">Agenda</h2><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Solicitudes</p></div>
        <button onClick={onLogout} className="bg-slate-100 p-2.5 rounded-xl text-slate-400 hover:text-red-500 transition-all"><Unlock size={18}/></button>
      </div>

      {pending.length === 0 ? <div className="py-24 text-center text-slate-300 font-black uppercase text-[10px] tracking-widest italic border-2 border-dashed border-slate-100 rounded-[3rem]">Sin pendientes</div> : 
        pending.map(app => (
          <div key={app.id} className="bg-white border border-slate-50 rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/20 space-y-6">
            <div className="flex justify-between items-start">
              <div><h3 className="font-black text-2xl tracking-tighter uppercase leading-none">{app.name} <span className="text-amber-600 font-light">{app.lastName}</span></h3><p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest">{app.phone}</p></div>
              <span className="bg-black text-amber-500 text-[9px] px-4 py-2 rounded-xl font-black uppercase italic tracking-wider shadow-lg shadow-black/10">{app.service}</span>
            </div>
            <div className="space-y-3 pt-2">
              <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest block ml-2">Confirmar disponibilidad:</span>
              {['slot1', 'slot2', 'slot3'].map((k, idx) => (
                <button key={k} onClick={() => confirmDate(app, k)} className="w-full flex justify-between items-center p-5 bg-slate-50 rounded-2xl text-[11px] font-black hover:bg-black hover:text-amber-500 transition-all border border-transparent hover:border-amber-500 group">
                  <span className="flex items-center gap-4"><span className="w-7 h-7 rounded-full bg-slate-200 text-slate-400 flex items-center justify-center text-[9px] font-black group-hover:bg-amber-500 group-hover:text-black transition-colors">{idx + 1}</span>{app[k] ? new Date(app[k]).toLocaleString('es-CR', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : '---'}</span>
