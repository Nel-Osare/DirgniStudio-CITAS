...
import { 
  ...
  ChevronRight, Crown, User, Clock, Lock, Unlock, X, Send, Phone
} from 'lucide-react';
...
export default function App() {
  ...
  // EFECTO 1: Autenticación Anónima (Obligatorio para Firebase)
  useEffect(() => {
  ...
  // EFECTO 2: Carga de citas en tiempo real (Solo para el Admin)
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Ordenar por las más nuevas primero
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-[3rem] p-10 shadow-2xl text-center border border-gray-100">
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-6">Introduce tu PIN táctico</p>
...
function ClientView() {
  ...
  const handleSubmit = async (e) => {
    ...
    const msg = `👑 ¡Hola Dirgni Studio!\nSoy *${form.name} ${form.lastName}*.\nSolicito cita para: *${form.service.toUpperCase()}*.\n\nPropuestas de Horario:\n1️⃣ ${fDate(form.s1)}\n2️⃣ ${fDate(form.s2)}\n3️⃣ ${fDate(form.s3)}`;

    // Redirección directa para que funcione en móviles sin bloqueos
    window.location.href = `https://wa.me/${SALON_PHONE}?text=${encodeURIComponent(msg)}`;
    setSent(true);

    // Guardar en base de datos para respaldo del Admin
    try {
      ...
      console.warn("Mensaje enviado, pero hubo un error de respaldo en la base de datos.");
    }
  };
  ...
  if (sent) return (
    <div className="text-center py-20 px-6 space-y-6 animate-in zoom-in duration-500">
      ...
      <h2 className="text-2xl font-black uppercase italic tracking-tighter">¡Cita Solicitada!</h2>
      <p className="text-gray-400 text-sm">Ya enviamos tu solicitud por WhatsApp. Revisa tu chat para confirmar los detalles finales.</p>
      <button onClick={() => setSent(false)} className="text-[10px] font-black uppercase underline text-gray-400 mt-4">Solicitar otra fecha</button>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-50 space-y-4">
        ...
        <input required type="tel" placeholder="WhatsApp (Celular)" className="w-full p-4 rounded-xl bg-gray-50 border-0 outline-none focus:ring-2 focus:ring-amber-500/20 font-medium" onChange={e => setForm({...form, phone: e.target.value})} />
      </div>
      ...
      <div className="bg-gray-900 p-8 rounded-[3rem] space-y-4 shadow-2xl relative overflow-hidden">
        <div className="flex items-center gap-2 mb-2 text-amber-500 font-black text-[11px] uppercase tracking-[0.2em] relative z-10">
...
function AdminView({ appointments, loading, onLogout }) {
  const confirmWithClient = (item, slotKey) => {
    ...
    const msg = `👑 ¡Hola *${item.name}*! Soy del equipo de *Dirgni Studio*.\n\nHe verificado tu solicitud y te CONFIRMO la cita para el día:\n📅 *${fDate}*\n\n¡Estamos listos para recibirte! ✨`;
    
    // Abrir WhatsApp para confirmar directamente a la clienta
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center px-2">
        <div>
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Solicitudes Entrantes ({pending.length})</p>
        </div>
        <button onClick={onLogout} className="bg-gray-100 p-3 rounded-2xl text-gray-400 hover:text-red-500 transition-all border border-gray-200">
          <Unlock size={20}/>
        </button>
      </div>
        ...
        pending.map(app => (
          <div key={app.id} className="bg-white border border-gray-100 rounded-[3rem] p-8 shadow-xl shadow-gray-200/20 space-y-6 animate-in slide-in-from-bottom-4">
            ...
            <div className="space-y-3 pt-4 border-t border-gray-50">
              <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest block ml-2">Selecciona una opción para confirmar:</span>
              {['s1', 's2', 's3'].map((k, idx) => (
                <button 
                  key={k} 
